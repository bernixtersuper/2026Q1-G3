resource "aws_s3_object" "glue_analytics_enrich_script" {
  bucket = module.s3_analytics_bronze.bucket_name
...
resource "aws_glue_job" "analytics_enrich" {
  ...
  command {
    script_location = "s3://${module.s3_analytics_bronze.bucket_name}/${aws_s3_object.glue_analytics_enrich_script.key}"
    python_version  = "3"
  }
  ...
  default_arguments = {
    ...
    "--BRONZE_BUCKET"                    = module.s3_analytics_bronze.bucket_name
    "--SILVER_BUCKET"                    = module.s3_analytics_silver.bucket_name
