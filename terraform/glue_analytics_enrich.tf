resource "aws_s3_object" "glue_analytics_enrich_script" {
  bucket = module.s3_analytics_events.bucket_name
...
resource "aws_glue_job" "analytics_enrich" {
  ...
  command {
    script_location = "s3://${module.s3_analytics_events.bucket_name}/${aws_s3_object.glue_analytics_enrich_script.key}"
    python_version  = "3"
  }
  ...
  default_arguments = {
    ...
    "--EVENTS_BUCKET"                    = module.s3_analytics_events.bucket_name
    "--ML_ANALYTICS_BUCKET"              = module.s3_analytics_ml.bucket_name
